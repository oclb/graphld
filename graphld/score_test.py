...
def _load_variant_data(hdf5_path: str) -> dict:
    ...
    with h5py.File(hdf5_path, 'r') as f:
        ...
        if 'variants' not in f or 'traits' not in f:
            raise ValueError(
                "HDF5 file "
                f"{hdf5_path} does not have the expected structure with "
                "'variants' and 'traits' groups"
            )
...
def _get_block_boundaries(blocks: np.ndarray) -> np.ndarray:
    ...
    if len(temp) != num_blocks - 1:
        raise ValueError(
            "Jackknife block indices in merged dataframe should be contiguous; "
            f"found {len(temp)} differences for {num_blocks} unique blocks"
        )
...
def _add_random_annotations(
    random_annotation_params: Optional[str],
    df_annot: pl.DataFrame,
) -> Tuple[pl.DataFrame, List[str]]:
    """Add random annotations based on parameters.

    Returns the modified DataFrame and list of added column names.
    """
    random_annot_names = []
...
def _run_score_test_jackknife(
    ...
    """
    Get jackknife score statistics for each annotation
    ...
    Args:
        ...
        adjust_uncertain_fit: Whether to adjust for uncertainty in the model fit when computing
            jackknife derivatives.
            Recommended to be False for exploratory analyses.



    Returns:
        Tuple of (point estimates, jackknife estimates)
    """
...
def run_score_test(df_snp: pl.DataFrame,
    ...
    U_total, U_jackknife = _run_score_test_jackknife(
        df_snp,
        df_annot,
        params,
        jk_params,
...
def main():
    parser = argparse.ArgumentParser(
        description='Run score test for annotation enrichment.'
    )

    parser.add_argument(
        "variant_stats_hdf5",
        help="Path to the HDF5 file containing precomputed derivatives."
    )
    parser.add_argument(
        "output_fp",
        help=("Output file path prefix (e.g., 'results/my_test'). "
              "'.txt' and '.log' will be appended.")
    )
    parser.add_argument(
        "-a", "--annotations_dir",
        default="data/annot",
        help="Directory containing annotation files to test (e.g., .annot, .bed)."
    )
    parser.add_argument(
        "-n", "--name",
        help="Specific trait name to process from HDF5 file. "
             "If omitted, all traits are processed."
    )
    parser.add_argument(
        "--annotations",
        help="Optional comma-separated list of specific annotation "
             "names (columns) to test."
    )
    parser.add_argument(
        "--add-random", dest="add_random",
        help=("Comma-separated list of probabilities (0-1) to generate "
              "random binary annotations for testing (e.g., '0.1,0.01').")
    )
    parser.add_argument(
        "--stream-stdout",
        dest="stream_stdout",
        action='store_true',
        help=('Stream log information on console in addition to '
              'writing to log file.')
    )
    parser.add_argument(
        "--seed",
    ...
    if not final_annot_test_list:
        logging.info(
            "No specific or random annotations provided. "
            "Detecting annotations from annotation file..."
        )
    ...
    with h5py.File(args.variant_stats_hdf5, 'r') as f:
        trait_names = [args.name] if args.name else list(f['traits'].keys())
    ...
    for trait in trait_names:
        ...
        point_estimates, jackknife_estimates = _run_score_test_jackknife(
        ...
        results_dict[trait] = point_estimates.ravel() / np.std(jackknife_estimates, axis=0) \
            / np.sqrt(jackknife_estimates.shape[0]-1)
