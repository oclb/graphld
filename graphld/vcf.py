...
def validate_vcf_format_columns(columns: list, verbose: bool = False) -> dict:
    ...
    Args:
        ...
        verbose (bool, optional): If True, print detailed information about columns. Defaults to False.
    ...
    vcf_format_spec = {
        'NS': {'description': 'Variant-specific number of samples/individuals with called genotypes used to test association with specified trait', 'required': False},
        'EZ': {'description': 'Z-score provided if it was used to derive the ES and SE fields', 'required': False},
        'SI': {'description': 'Accuracy score of association statistics imputation', 'required': False},
        'NC': {'description': 'Variant-specific number of cases used to estimate genetic effect (binary traits only)', 'required': False},
        'ES': {'description': 'Effect size estimate relative to the alternative allele', 'required': True},
        'SE': {'description': 'Standard error of effect size estimate', 'required': True},
        'LP': {'description': '-log10 p-value for effect estimate', 'required': True},
        'AF': {'description': 'Alternative allele frequency in trait subset', 'required': False},
        'AC': {'description': 'Alternative allele count in the trait subset', 'required': False}
    }
    
    # Check for missing required columns
    missing_required = [col for col, spec in vcf_format_spec.items() if spec['required'] and col not in columns]
    if missing_required:
    ...
    if extra_columns:
        raise ValueError(f"Extra columns not in VCF FORMAT specification: {extra_columns}")
    ...
    if verbose:
        ...
        for col in columns:
            ...
            print(f"- {col}: {spec['description']} ({'Required' if spec['required'] else 'Optional'})")
