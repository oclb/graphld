...
- Clarified that score-test gene annotation loading keeps gene-table and nearest-weight parameters for compatibility, but only loads GMT gene sets before later conversion steps.
...
- Removed stale inline TODO markers, the unused score-test `GenomeAnnot` stub, and the unreferenced duplicate `heritability_testing.py` module.
