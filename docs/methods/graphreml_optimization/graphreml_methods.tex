...
\section{Baseline-LD analysis of the previous trait cohort}
We analyze the fixed set of 33 traits used in the previous burden-study presentation (27 continuous and six binary traits), preserving the phenotype mappings and variant targets. The unpenalized AI fits use 97 baseline-LD annotations, trait-specific sample sizes and fixed S-LDSC intercepts, and the same neutral initialization. Variants have reference minor-allele frequency above 1\%, with the MHC excluded. The frequency exponent $\alpha$ is obtained from the fitted heritability in ten frequency bins above 5\% reference frequency, using the exact frequency distribution within each bin: a variant's predicted heritability is proportional to $[2p(1-p)]^{1+\alpha}$. This is a downstream covariance-aware fit to partitioned heritability. Nonsynonymous enrichment is the fraction of heritability assigned to the binary \texttt{non\_synonymous} annotation divided by its fraction of analyzed variants; it differs from the broader Coding category in the six-annotation comparison.

All 33 fits finished. Thirteen passed the precise original-information stopping checks and the uncertainty and extraction checks (Table~\ref{tab:cohort}); nineteen reached the 100-iteration limit. Systolic blood pressure passed the score and signed-gain checks but had information rank 96/97. Acute sinusitis both reached the iteration limit and had information rank 96/97. Alpha and uncertainty were withheld for these two singular-information outcomes. All thirteen validated fits were continuous traits; all six binary traits reached the iteration limit. For the six binary traits, the new endpoint log likelihoods were 55.8--113.1 below the historical endpoints after verifying identical full variant targets, sample sizes, intercepts and annotation models. This shows a remaining optimization limitation; the historical stopping flags and boundary-affected outputs do not establish usable inference. The accompanying aggregate table retains all 33 outcomes, provisional point estimates where available, uncertainty qualifications and timing definitions. These local checks do not establish a global likelihood maximum.

\begin{table}[H]
\centering
\small
\input{cohort_final/validated_estimates_table.tex}
\caption{Validated baseline-LD results for thirteen traits. Enrichment refers to nonsynonymous variants. Parentheses give standard errors from 100 ordered one-step delete groups. Alpha fits use the fixed full-fit covariance and exact frequency quadrature; numerical checks do not assess uncertainty calibration. All fits shown passed the unpenalized local stationarity and information checks. The twenty unresolved outcomes are retained in the aggregate source table.}
\label{tab:cohort}
\end{table}

For these summaries, the covariance of a vector of estimates is $(B-1)/B$ times the centered cross-product of the $B=100$ delete estimates. Alpha is fitted again in each delete using the full-fit bin covariance. This canonical covariance normalization gives standard errors smaller than the native normalization by $\sqrt{B/(B-1)}$. Variant ordering, alleles, coordinates, annotations, frequency quadrature and delete-group membership were checked before accepting results. Independent alpha optimization repeated the full fit and all 100 delete fits for ten of the thirteen validated traits at tolerance $10^{-6}$. BMI, Weight and Hemoglobin instead have benchmark-parity and saved covariance/arithmetic checks; those records do not independently certify their alpha optima. Historical endpoints that failed stationarity remain descriptive comparators; their runtimes do not establish performance at matched accuracy.

The preserved binary phenotype mappings vary in power and specificity. Acute sinusitis uses the J01 main-ICD10 phenotype with 112 cases; obesity has 448 cases. Hypertension and hyperlipidemia use self-reported phenotypes, while breast cancer uses the female sample. Thus membership in the previous presentation does not imply that each current summary-statistic mapping is well powered. Binary analyses retain the original association $Z$ statistics, total sample sizes and fixed intercepts; effective sample size is descriptive metadata, and no liability-scale conversion is applied.

Unresolved fits can yield misleadingly small or large uncertainty estimates. For cellulitis, all 100 delete alpha estimates equal the lower bound $-4$, giving a mechanical standard error of zero despite an unresolved full estimate of $-2.801$. For obesity, 50 delete alpha estimates hit a bound; the full estimate is $-0.660$, its standard error is 26.899, and its profile interval is unavailable. Its nonsynonymous enrichment is 0.243 with standard error 72.197. These outputs remain explicitly provisional. Mean corpuscular volume also failed a delete-alpha check using independently reconstructed covariance at the unchanged numerical tolerance, although its saved-output reconstruction was reproducible.

Recorded optimizer times across the 33 cohort runs ranged from 35.9 to 170.0 minutes (median 84.1); among the thirteen validated fits the median was 64.9 minutes. Each fit used twelve workers on the local M4 Max machine, with one numerical-library thread per worker. Early runs experienced concurrent workloads, and later primary fits ran serially. These measurements describe this execution history, rather than a controlled comparison of optimizers. The aggregate table distinguishes optimizer time, the complete native API call, input preparation and runner time through preparation of native outputs, before downstream extraction; independent post-fit audits are excluded. The matched comparisons above remain the evidence for relative runtime.

\subsection{Limits identified by convergence experiments}
Across the final ten iterations of each of the thirteen capped continuous-trait fits, all 130 trials were accepted. Ten traits used a scaled-gradient proposal throughout this interval, red-cell count used it in nine iterations, and mean platelet volume and triglycerides used AI throughout. Repeated backtracking is therefore not the observed late-stage bottleneck in this subset. Saturated weak-information coordinates were shown to force restrictive global clipping in six separately audited traits; the proposal logs alone do not establish that mechanism for every capped trait.

An experimental constrained-AI method maximized the original quadratic under bounds on each variant's predictor displacement. It exposed substantial feasible ascent at difficult endpoints, but its local stopping checks did not ensure comparable final likelihood. In a matched-worker BMI check, a candidate endpoint admitted a feasible displacement with likelihood gain 0.06087 while its quadratic predicted a decrease of 0.38859. Candidate endpoint likelihood remained approximately 0.239 below the implemented AI comparator, exceeding the 0.001 comparison tolerance. Agreement of precise directional scores with finite differences at tested points did not eliminate this finite-step discrepancy. A bounded local quadratic criterion and a few signed probes therefore remain local diagnostics.

We also compared AI with half and full subtraction of the nonlinear link-curvature term from its proposal information, applying the same constrained quadratic solver to positive-definite proposal matrices and falling back to AI otherwise. In five fixed synthetic examples with known maximum-likelihood targets, all three methods reached comparable acceptable endpoints. Across the four nonlinear examples, AI required 79 likelihood or derivative calls, the half correction 87, and the full correction 80. The full correction saved three calls in a negative-link-curvature example but added four in a nearly collinear example. Both failed the prespecified efficiency criterion. Small synthetic timings do not establish biological runtime; derivative and likelihood calls also have different costs. These experimental results provide no basis for changing the implemented optimizer or claiming reliable convergence across the full trait cohort.
