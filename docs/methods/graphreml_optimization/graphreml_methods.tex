...
\section{Interim baseline-LD trait analysis}
We analyze the fixed set of 33 well-powered burden-study traits (27 continuous and six binary traits), preserving the phenotype mappings and variant targets. The unpenalized AI fits use 97 baseline-LD annotations, trait-specific sample sizes and fixed S-LDSC intercepts, and the same neutral initialization. Variants have reference minor-allele frequency above 1\%, with the MHC excluded. The frequency exponent $\alpha$ is obtained from the fitted heritability in ten frequency bins above 5\% reference frequency, using the exact frequency distribution within each bin: a variant's predicted heritability is proportional to $[2p(1-p)]^{1+\alpha}$. This is a downstream covariance-aware fit to partitioned heritability. Nonsynonymous enrichment is the fraction of heritability assigned to the binary \texttt{non\_synonymous} annotation divided by its fraction of analyzed variants; it differs from the broader Coding category in the six-annotation comparison.

At the September 13, 2026, 16:33 UTC snapshot, 24 fits had finished and nine were pending or running. Ten passed the precise original-information stopping checks and the uncertainty and extraction checks (Table~\ref{tab:cohortinterim}); thirteen reached 100 iterations with the precise criterion still above tolerance. Systolic blood pressure passed the score and signed-gain checks but had information rank 96/97, so its uncertainty and alpha were withheld. All 33 outcomes, including unresolved and pending rows, are retained in the accompanying aggregate table. This completion-order subset does not estimate the eventual cohort-wide convergence rate.

\begin{table}[H]
\centering
\small
\input{cohort_interim/validated_estimates_table.tex}
\caption{Interim validated baseline-LD results for ten traits. Enrichment refers to nonsynonymous variants. Parentheses give standard errors from 100 ordered one-step delete groups. Full and delete alpha fits were independently repeated using the fixed full-fit covariance and exact frequency quadrature; these checks assess numerical reproduction, not uncertainty calibration. All fits shown passed the unpenalized local stationarity and information checks. The nine unfinished and fourteen unresolved outcomes are retained in the aggregate source table.}
\label{tab:cohortinterim}
\end{table}

For these summaries, the covariance of a vector of estimates is $(B-1)/B$ times the centered cross-product of the $B=100$ delete estimates. Alpha is fitted again in each delete using the full-fit bin covariance. This canonical covariance normalization gives standard errors smaller than the native normalization by $\sqrt{B/(B-1)}$. Variant ordering, alleles, coordinates, annotations, frequency quadrature and delete-group membership were checked before accepting results. Historical endpoints that failed stationarity remain descriptive comparators; their runtimes do not establish performance at matched accuracy.

\subsection{Limits identified by convergence experiments}
Across the final ten iterations of each of the thirteen capped fits, all 130 trials were accepted. Ten traits used a scaled-gradient proposal throughout this interval, red-cell count used it in nine iterations, and mean platelet volume and triglycerides used AI throughout. Repeated backtracking is therefore not the observed late-stage bottleneck. Saturated weak-information coordinates were shown to force restrictive global clipping in six separately audited traits; the proposal logs alone do not establish that mechanism for every capped trait.
